...
const Sidebar = (props:{filterProducts: Function}) => {
  const [filters, setFilters] = useState<Filter>({ manufacturer:[]});

  const handleFilter = (e: React.ChangeEvent<HTMLInputElement>) => {
    const isChecked: boolean = e.target.checked;
    const name: string = e.target.name;
    console.log("Checked: ", isChecked);
    //This goes in onsubmit
    // filterProducts()
      setFilters({ ...filters, manufacturer:[...filters?.manufacturer, name]});
        ...
        <div className="h-full px-3 py-4 overflow-y-auto bg-gray-50 dark:bg-gray-800">
          ...
          <ul className="space-y-2 ml-8">
            {testData.map((manufacturer, index) => {
              return (
                <li key={index}>
                  <input type="checkbox" name = {manufacturer} onClick={(e) => handleFilter(e)} />
